 ...
 * Ported faithfully from .local/design-assets/dst-flow.jsx. Markup, copy, SVG,
 * inline styles and animations are byte-faithful to the design; only data inputs
 * change (typed props + the real-backend `notice` retry path on FramingScreen).
 * PrimaryBtn / ArrowR live in ./atoms (imported, never redefined). */

import { useEffect, useState, type RefObject } from "react";
import { FaceMap, Dot, Chip, PrimaryBtn, ArrowR } from "./atoms";
import type { CamStatus, LightState } from "./engine";
import { FIX_HINTS, type ScreenCheck } from "../lib";
...
export function FramingScreen({
  ...
  onCapture: () => void;
  onBack: () => void;
  /** Failing checks when the user was bounced back from an unusable analyze.
   *  Rendered above the controls via each check's message or FIX_HINTS[id]. */
  notice?: ScreenCheck[];
}) {
  ...
  const LIGHT_COPY =
    (
      ...
      } as Record<string, { tone: 'neutral' | 'good' | 'warn'; text: string }>
    )[ls] || { tone: 'neutral' as const, text: 'Reading…' };

  // The failing checks to surface on a retry. Prefer those explicitly marked as
  // failed; fall back to the whole set if the backend didn't flag pass=false.
  const failing = (notice ?? []).filter((c) => c.pass === false);
  const noticeChecks = failing.length ? failing : notice ?? [];
  ...
  return (
    <div style={{ height: '100%', background: '#15110f', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        ...
        {/* retry notice — failing checks from an unusable analyze (real-backend path) */}
        {noticeChecks.length > 0 && (
          <div style={{ position: 'absolute', top: 108, left: 16, right: 16, display: 'flex', justifyContent: 'center' }}>
            <div style={{ background: 'rgba(245,176,82,0.16)', border: '1px solid rgba(245,176,82,0.5)', backdropFilter: 'blur(16px)',
              ...
              <div style={{ fontFamily: 'Instrument Serif, serif', fontStyle: 'italic', fontSize: 16, color: '#ffd9a3', textAlign: 'center' }}>That shot wasn&rsquo;t usable.</div>
              <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
                {noticeChecks.map((c) => (
                  <div key={c.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 7, fontFamily: 'Switzer, sans-serif', fontSize: 11.5, lineHeight: 1.4, color: 'rgba(255,217,163,0.9)' }}>
                    <span style={{ flex: 'none', width: 4, height: 4, borderRadius: 999, background: '#f5b052', marginTop: 6 }} />
                    <span>{c.message || FIX_HINTS[c.id] || c.label}</span>
                  </div>
                ))}
              </div>
            </div>
...
export function ScanningScreen({
  ...
  const [lTally, setLTally] = useState(0);
  const [rTally, setRTally] = useState(0);
