 ...
 * Ported faithfully from design-assets/dst-result.jsx (markup / copy / SVG /
 * animation kept byte-for-byte) with the synthetic data inputs swapped for the
 * REAL UV Mirror backend payloads the integrator threads through DriverFlow:
 *   - BreakdownScreen renders the real `overall` severity + per-region scores.
 *   - EmailScreen exposes async lead state (`submitting` / `error`).
 *   - AppStoreScreen surfaces the PDF report link when a `reportToken` exists.
 */

import { useState, useEffect, useRef } from "react";
import type { ReactNode, PointerEvent as ReactPointerEvent } from "react";
import { PrimaryBtn, ArrowR, FaceMap, Dot } from "./atoms";
import { EMAIL_RE, APP_DOWNLOAD_URL, reportUrl } from "../lib";
import type { Overall, Region } from "../lib";
...
interface RevealScreenProps {
  ...
  damagedURL: string | null;
  asymmetry: number; // 0..100 int
  sunAge: number;
  revealStyle: "wipe" | "split" | "silhouette";
  tone: "calm" | "dramatic";
  onNext: () => void;
}

export function RevealScreen({ cleanURL, damagedURL, asymmetry, sunAge, revealStyle, tone, onNext }: RevealScreenProps) {
  ...
  useEffect(() => {
    ...
    const done = setTimeout(() => {
      if (!touchedRef.current) setSeam(rest);
      setSettled(true);
    }, dur + 250);
    return () => {
      if (raf) cancelAnimationFrame(raf);
      ...
      <div ref={wrapRef} onPointerDown={onPointer} style={{ flex: 1, position: "relative", overflow: "hidden", cursor: revealStyle === "split" ? "default" : "ew-resize", touchAction: "none" }}>
        ...
        <div style={{ position: "absolute", left: 0, right: 0, bottom: 18, padding: "0 22px", pointerEvents: "none" }}>
          <div style={{ fontFamily: "Instrument Serif, serif", fontStyle: "italic", fontSize: 19, color: "#fff", lineHeight: 1.2, marginBottom: 12, maxWidth: 280 }}>
            {dramatic ? (
              <>Your driver&rsquo;s side is carrying <span style={{ color: "var(--accent2)" }}>most of the sun</span>.</>
            ) : (
              <>A real, <span style={{ color: "var(--accent2)" }}>measurable</span> lean to one side.</>
            )}
          </div>
...
interface RevealTagProps {
  title: string;
  sub: string;
  tone?: "hot";
}

export function RevealTag({ title, sub, tone }: RevealTagProps) {
  const hot = tone === "hot";
  return (
...
function ResultStat({ big, unit, label, accent }: ResultStatProps) {
  return (
...
interface BreakdownScreenProps {
  ...
  tone: "calm" | "dramatic";
  onNext: () => void;
  overall: Overall; // REAL headline severity / score
  regions: Region[]; // REAL per-region scores
}

export function BreakdownScreen({ asymmetry, sunAge, tone, onNext, overall, regions }: BreakdownScreenProps) {
  const base = asymmetry / 100;

  // Surface the most sun-exposed zones first — faithful to the design's
  // descending presentation, now driven by the real per-region scores.
  const ranked = [...regions].sort((a, b) => b.score - a.score);
  const topLabels = ranked.slice(0, 2).map((r) => r.label);
  const topPhrase = topLabels.length === 2 ? `${topLabels[0]} and ${topLabels[1]}` : topLabels[0] || "driver\u2019s side";

  const sevColor = overall.severity === "high" ? "var(--accent)" : overall.severity === "moderate" ? "var(--accent2)" : "var(--muted)";

  // subject's left side faces the driver's window → "driver's side" (accent);
  // right → passenger (glow); center → neutral warm tone.
  const sideColor = (side: Region["side"]) => (side === "left" ? "var(--accent)" : side === "right" ? "var(--glow)" : "var(--accent2)");
  const sideLabel = (side: Region["side"]) => (side === "left" ? "L" : side === "right" ? "R" : "C");
  ...
  return (
    <div style={{ height: "100%", background: "var(--bg)", overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
      <div style={{ padding: "40px 24px 8px", display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
        ...
        {/* REAL headline metric: overall sun-damage score + severity band */}
        <div style={{ textAlign: "right", flexShrink: 0, paddingLeft: 14 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 3, justifyContent: "flex-end" }}>
            <span style={{ fontFamily: "Switzer, sans-serif", fontSize: 38, fontWeight: 400, color: "var(--ink)", lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>{overall.sunDamageScore}</span>
            <span style={{ fontFamily: "Switzer, sans-serif", fontSize: 13, color: "var(--muted)" }}>/ 100</span>
          </div>
          <div style={{ fontFamily: "Switzer, sans-serif", fontSize: 10, letterSpacing: 0.6, textTransform: "uppercase", color: sevColor, marginTop: 4 }}>{overall.severity} sun damage</div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "16px 24px 4px" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontFamily: "Switzer, sans-serif", fontSize: 13, color: "var(--muted)", lineHeight: 1.5 }}>
            Higher UV-exposure index = more cumulative sun. Your sun load is heaviest at the <em style={{ fontFamily: "Instrument Serif, serif", fontStyle: "italic", color: "var(--ink)" }}>{topPhrase}</em>.
          </div>
        </div>
      </div>

      {/* zone rows — one per real region, scored by side */}
      <div style={{ padding: "12px 16px 0", display: "flex", flexDirection: "column", gap: 8 }}>
        {ranked.map((r) => {
          const hot = r.score >= 66;
          return (
            <div key={r.id} style={{ background: "var(--surface)", border: "1px solid var(--glow)", borderRadius: 16, padding: "13px 15px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 9 }}>
                <span style={{ fontFamily: "Switzer, sans-serif", fontSize: 14, fontWeight: 600, color: "var(--ink)" }}>{r.label}</span>
                <span style={{ fontFamily: "Switzer, sans-serif", fontSize: 11, fontWeight: 600, color: hot ? "var(--accent)" : "var(--muted)" }}>{r.spotCount} spot{r.spotCount === 1 ? "" : "s"}</span>
              </div>
              <ZoneBar label={sideLabel(r.side)} value={r.score} max={100} c={sideColor(r.side)} dark={r.side === "right"} />
            </div>
          );
        })}
      </div>
...
function Legend({ c, t }: LegendProps) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <Dot c={c} />
      <span style={{ fontFamily: "Switzer, sans-serif", fontSize: 10, letterSpacing: 0.6, textTransform: "uppercase", color: "var(--muted)" }}>{t}</span>
    </div>
  );
}
...
function ZoneBar({ label, value, max, c, dark }: ZoneBarProps) {
  return (
...
interface EmailScreenProps {
  ...
  onSkip: () => void;
  submitting?: boolean; // NEW: lead request in flight (machine performs the call)
  error?: string | null; // NEW: inline lead-failure message
}

export function EmailScreen({ asymmetry, sunAge, onSubmit, onSkip, submitting = false, error = null }: EmailScreenProps) {
  const [email, setEmail] = useState("");
  const valid = EMAIL_RE.test(email);
  const canSend = valid && !submitting;
  return (
    <div style={{ height: "100%", background: "var(--bg)", overflowY: "auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "20px 22px 0", marginTop: "auto" }}>
        <div style={{ display: "flex", alignItems: "center", background: "var(--surface)", border: `1px solid ${valid ? "var(--accent)" : "var(--glow)"}`, borderRadius: 999, padding: "5px 5px 5px 18px", transition: "border-color 200ms" }}>
          <input value={email} onChange={(e) => setEmail(e.target.value)} disabled={submitting} type="email" placeholder="you@email.com" style={{ flex: 1, border: "none", outline: "none", background: "transparent", fontFamily: "Switzer, sans-serif", fontSize: 15, color: "var(--ink)", padding: "11px 6px" }} />
          <button onClick={canSend ? () => onSubmit(email) : undefined} disabled={!canSend} style={{
            border: "none", borderRadius: 999, padding: "11px 18px", fontFamily: "Switzer, sans-serif", fontSize: 14, fontWeight: 600,
            background: "var(--ink)", color: "var(--surface)", cursor: canSend ? "pointer" : "not-allowed", opacity: canSend ? 1 : 0.4, whiteSpace: "nowrap" }}>
            {submitting ? "Sending\u2026" : "Send it"}
          </button>
        </div>
        {error && (
          <div style={{ textAlign: "center", fontFamily: "Switzer, sans-serif", fontSize: 12.5, color: "#c0473e", marginTop: 10, lineHeight: 1.5 }}>{error}</div>
        )}
...
interface AppStoreScreenProps {
  ...
  email: string;
  onRestart: () => void;
  reportToken?: string | null; // NEW: when present, surface the PDF report link
}

export function AppStoreScreen({ sent, email, onRestart, reportToken }: AppStoreScreenProps) {
  return (
    <div style={{ height: "100%", background: "var(--bg)", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "46px 28px 32px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", display: "flex", flexDirection: "column", gap: 10 }}>
        {reportToken && (
          <a href={reportUrl(reportToken)} target="_blank" rel="noreferrer" style={{
            ...
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3v12" /><path d="M7 11l5 5 5-5" /><path d="M5 21h14" /></svg>
            Download your PDF report
          </a>
        )}
        <PrimaryBtn style={{ width: "100%" }} onClick={() => window.open(APP_DOWNLOAD_URL, "_blank", "noopener,noreferrer")}>
