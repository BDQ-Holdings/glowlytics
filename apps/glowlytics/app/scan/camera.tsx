...
import { Colors, FontFamily, FontSize, BorderRadius, Glow, Spacing } from '../../src/constants/theme';
...
export default function CameraScreen() {
  ...
  // Track the underlying status so we can distinguish "never asked" from "user denied" —
  // iOS won't re-prompt when status is denied/restricted, so requestPermission() silently
  ...
  // Workaround for vision-camera #2143: defer isActive=true to after mount + a tick so
  // layout/permission state has fully settled before the native AVCaptureSession opens.
  ...
  if (!hasPermission) {
    const isPermanentlyDenied = permissionStatus === 'denied' || permissionStatus === 'restricted';
    return (
      <View style={styles.permissionContainer}>
        <View style={styles.permissionContent}>
          <GlowIcon name="camera" size={48} color={Glow.palette.accent} stroke={1.5} />
          <Text style={styles.permissionTitle}>
            <Text style={{ fontFamily: FontFamily.accent, fontSize: 36 }}>Camera</Text> access
          </Text>
    ...
    return (
      <View style={styles.permissionContainer}>
        <View style={styles.permissionContent}>
          ...
          <Text style={styles.permissionTitle}>
            <Text style={{ fontFamily: FontFamily.accent, fontSize: 36 }}>No</Text> camera
          </Text>
          <Text style={styles.permissionText}>
  ...
  return (
    <View style={styles.container}>
      <Camera
        ...
        isActive={cameraSessionArmed && !paywallVisible}
        photo={true}
            ...
            <Animated.View style={buttonAnimStyle}>
              <View style={[
                styles.captureRing,
                {
                  ...
                  shadowColor: trackingState.status === 'aligned' ? Glow.palette.accent : 'transparent',
                  shadowOpacity: 0.7,
                  shadowRadius: 24,
