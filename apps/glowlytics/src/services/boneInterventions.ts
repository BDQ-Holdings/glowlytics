 ...
 *
 * Direct TypeScript port of `backend/interventions.js`. Pure lookup data plus
 * a deduplication pass — no I/O, no network. Runs on-device alongside the
 * `onDeviceBoneStructure` analyzer to produce the full `InterventionBundle`
 * the UI renders on the bone-results screen.
 *
...
type SuggestionEntry = { tier: BoneInterventionTier; title: string; body: string };

export const SUGGESTIONS: Record<string, SuggestionEntry> = {
  // ----- LIFESTYLE -----
...
export const FINDING_TO_SUGGESTIONS: Partial<Record<BoneFindingCode, TierMap>> = {
  // ---- Periorbital ----
...
export const PROCEDURAL_DISCLAIMER =
...
export function recommendInterventions(
  findings: BoneFinding[] | null | undefined,
  { perTierCap = 6 }: { perTierCap?: number } = {},
): InterventionBundle {
  const lifestyle: BoneIntervention[] = [];
  const pharmacological: BoneIntervention[] = [];
  const interventional: BoneIntervention[] = [];
  const seen = {
    lifestyle: new Set<string>(),
    pharmacological: new Set<string>(),
    interventional: new Set<string>(),
  };

  const append = (tier: BoneInterventionTier, id: string) => {
    const bucket =
