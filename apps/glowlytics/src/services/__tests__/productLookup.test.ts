...
//
// httpClient.httpJson — which now sits behind productLookup's direct fetches —
// reads `res.ok`, `res.status`, `res.headers.get('content-type')` and either
// `res.json()` or `res.text()`. The legacy test mocks only set a subset of
// these, so we wrap raw mock data with a Response-shaped helper to keep the
// tests focused on behaviour rather than the wire format.
...
describe('lookupOpenBeautyFacts', () => {
  ...
  it('returns null when product not found', async () => {
    mockFetch.mockResolvedValueOnce(mockResponse({ json: { status: 0 } }));
...
describe('lookupUPCitemdb', () => {
  it('returns product when found', async () => {
    mockFetch.mockResolvedValueOnce(mockResponse({
      json: { items: [{ title: 'Neutrogena Hydro Boost' }] },
    }));
  ...
  it('returns null when no items found', async () => {
    mockFetch.mockResolvedValueOnce(mockResponse({ json: { items: [] } }));
