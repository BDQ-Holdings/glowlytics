...
type LandmarkKey =
  | 'trichion' | 'glabella' | 'pronasale' | 'subnasale' | 'menton' | 'pogonion'
  | 'upper_lip_top' | 'stomion' | 'lower_lip_bot'
  | 'inner_canthus_L' | 'inner_canthus_R' | 'outer_canthus_L' | 'outer_canthus_R'
  | 'upper_eyelid_L' | 'upper_eyelid_R' | 'lower_eyelid_L' | 'lower_eyelid_R'
  | 'brow_inner_L' | 'brow_inner_R' | 'brow_apex_L' | 'brow_apex_R'
  | 'alar_L' | 'alar_R' | 'zygion_L' | 'zygion_R' | 'tragion_L' | 'tragion_R'
  | 'gonion_L' | 'gonion_R' | 'cheilion_L' | 'cheilion_R' | 'iris_L' | 'iris_R';

type LandmarkTable = Partial<Record<LandmarkKey, number>>;
type LandmarkMap = Partial<Record<LandmarkKey, Vec3>>;

type Confidence = 'high' | 'medium' | 'low';
type LandmarkSource = 'indexed' | 'derived' | 'template';
...
interface LandmarkResolution {
  status: 'ok' | 'no_face';
  landmarks: LandmarkMap;
  confidence: Confidence;
  landmark_source: LandmarkSource;
  missingLoops?: boolean;
}
...
export const LANDMARK_TABLES: Record<'canonical' | 'mediapipe', LandmarkTable> = {
...
function allVertices(vertices: ArrayLike<number>): Vec3[] {
  const out: Vec3[] = [];
  for (let i = 0; i + 2 < vertices.length; i += 3) {
    const p = { x: vertices[i], y: vertices[i + 1], z: vertices[i + 2] };
    if (Number.isFinite(p.x) && Number.isFinite(p.y) && Number.isFinite(p.z)) out.push(p);
  }
  return out;
}

function extent(points: Vec3[]) {
...
function pickMax(points: Vec3[], score: (p: Vec3) => number): Vec3 | undefined {
...
function pickMin(points: Vec3[], score: (p: Vec3) => number): Vec3 | undefined {
...
export function isDegenerateMesh(vertices: ArrayLike<number> | null | undefined): boolean {
...
function normalizeSource(source: BoneMeshSource | 'canonical' | undefined): 'arkit' | 'canonical' | 'mediapipe' {
  if (source === 'arkit') return 'arkit';
  if (source === 'canonical') return 'canonical';
  if (source === 'mediapipe' || source == null) return 'mediapipe';
  throw new Error(`Unknown landmark source: ${source}`);
}

export function isSourceCoherent(
  source: BoneMeshSource | 'canonical' | undefined,
  vertexCount: number,
  indices?: ArrayLike<number> | null,
): { ok: boolean; reason?: string } {
  const normalized = normalizeSource(source);
  if (normalized === 'arkit') {
    if (vertexCount < 900) return { ok: false, reason: 'arkit source requires an ARKit-sized mesh (>=900 vertices)' };
    if (!indices || indices.length < 3) return { ok: false, reason: 'arkit source requires triangle indices for derived landmarks' };
    return { ok: true };
  }
...
function indexedLandmarks(vertices: ArrayLike<number>, source: 'canonical' | 'mediapipe'): LandmarkResolution {
  const table = LANDMARK_TABLES[source];
  const landmarks: LandmarkMap = {};
  for (const [key, idx] of Object.entries(table) as Array<[LandmarkKey, number | undefined]>) {
    const p = vec(vertices, idx);
    if (p) landmarks[key] = p;
  }
  return { status: 'ok', landmarks, confidence: 'medium', landmark_source: 'template' };
}

interface BoundaryLoop { points: Vec3[]; centroid: Vec3; xSpan: number; ySpan: number; length: number; }

function boundaryLoops(vertices: ArrayLike<number>, indices?: ArrayLike<number> | null): BoundaryLoop[] {
  ...
  for (const start of adjacency.keys()) {
    ...
    for (const firstNext of nexts) {
      if (visited.has(edgeKey(start, firstNext))) continue;
      const ids = [start];
      let prev = start;
      let cur = firstNext;
      ...
      const points = ids.map((id) => vec(vertices, id)).filter((p): p is Vec3 => !!p);
  ...
  const unique: BoundaryLoop[] = [];
  for (const loop of loops) {
    if (!unique.some((u) => distance(u.centroid, loop.centroid) < 1e-6 && Math.abs(u.length - loop.length) < 1)) unique.push(loop);
  }
  return unique.sort((a, b) => b.length - a.length);
}

function classifyFeatureLoops(loops: BoundaryLoop[], faceHeight: number): { leftEye?: BoundaryLoop; rightEye?: BoundaryLoop; mouth?: BoundaryLoop } {
  ...
  const mouth = nonOuter
    .filter((l) => l.centroid.y < (eyes[0]?.centroid.y ?? 0) - faceHeight * 0.18)
    .sort((a, b) => b.xSpan - a.xSpan)[0];
  return { leftEye: eyes[0], rightEye: eyes[1], mouth };
}

function loopExtremes(loop: BoundaryLoop) {
  return {
...
function pickByBand(points: Vec3[], predicate: (p: Vec3) => boolean, score: (p: Vec3) => number): Vec3 | undefined {
  return pickMax(points.filter(predicate), score);
}

export function deriveLandmarksFromMesh(vertices: ArrayLike<number>, indices?: ArrayLike<number> | null): LandmarkResolution {
  const points = allVertices(vertices);
  if (points.length < 3 || isDegenerateMesh(vertices)) return { status: 'no_face', landmarks: {}, confidence: 'low', landmark_source: 'derived' };
  const ex = extent(points);
  ...
  if (profile.length < 5) return { status: 'no_face', landmarks: {}, confidence: 'low', landmark_source: 'derived' };

  const menton = pickMin(profile, (p) => p.y);
  const trichion = pickMax(profile, (p) => p.y);
  const pronasale = pickMax(profile, (p) => p.z);
  if (!menton || !trichion || !pronasale) return { status: 'no_face', landmarks: {}, confidence: 'low', landmark_source: 'derived' };
  ...
  if (!glabella || !subnasale) return { status: 'no_face', landmarks: {}, confidence: 'low', landmark_source: 'derived' };
  ...
  const canthusY = ((leftEye?.centroid.y ?? 0) + (rightEye?.centroid.y ?? 0)) / (leftEye && rightEye ? 2 : 1) || (glabella.y * 0.7 + subnasale.y * 0.3);
  ...
  const landmarks: LandmarkMap = { trichion, glabella, pronasale, subnasale, menton, pogonion, zygion_L, zygion_R, gonion_L, gonion_R, tragion_L, tragion_R };
...
export interface FaceFrame { origin: Vec3; right: Vec3; up: Vec3; forward: Vec3; }

export function buildFaceFrameFromLandmarks(L: LandmarkMap): FaceFrame | null {
...
export function buildFaceFrame(vertices: ArrayLike<number>, L: LandmarkTable): FaceFrame | null {
  const landmarks: LandmarkMap = {};
  for (const [key, idx] of Object.entries(L) as Array<[LandmarkKey, number | undefined]>) {
    const p = vec(vertices, idx);
    if (p) landmarks[key] = p;
  }
...
function expressionIsNonNeutral(blendShapes: Record<string, number> | null | undefined): boolean {
  if (!blendShapes) return false;
  const jawOpen = blendShapes.jawOpen ?? 0;
  const smile = Math.max(blendShapes.mouthSmile_L ?? blendShapes.mouthSmileLeft ?? 0, blendShapes.mouthSmile_R ?? blendShapes.mouthSmileRight ?? 0);
  const blink = Math.max(blendShapes.eyeBlink_L ?? blendShapes.eyeBlinkLeft ?? 0, blendShapes.eyeBlink_R ?? blendShapes.eyeBlinkRight ?? 0);
  return jawOpen > 0.25 || smile > 0.4 || blink > 0.5;
}

function resolveLandmarks(
  vertices: ArrayLike<number>,
  source: 'arkit' | 'canonical' | 'mediapipe',
  indices?: ArrayLike<number> | null,
): LandmarkResolution {
  if (isDegenerateMesh(vertices)) return { status: 'no_face', landmarks: {}, confidence: 'low', landmark_source: source === 'arkit' ? 'derived' : 'template' };
  const vertexCount = Math.floor(vertices.length / 3);
  const coherent = isSourceCoherent(source, vertexCount, indices);
  if (!coherent.ok) return { status: 'no_face', landmarks: {}, confidence: 'low', landmark_source: source === 'arkit' ? 'derived' : 'template' };
  if (source === 'arkit') return deriveLandmarksFromMesh(vertices, indices);
...
export function computeMetrics3D(
  ...
  source: BoneMeshSource | 'canonical' = 'arkit',
  indices?: ArrayLike<number> | null,
...
function computeMetricsFromLandmarks(
  sourceLandmarks: LandmarkMap,
  blendShapes: Record<string, number> | null,
  confidence: Confidence,
  missingLoops: boolean,
): Record<string, MetricEntry> | null {
  ...
  const lp = (key: LandmarkKey): Vec3 | null => sourceLandmarks[key] ? toLocal(frame, sourceLandmarks[key]!) : null;
  ...
  const midline = [trichion, glabella, pronasale, subnasale, menton, pogonion, upperLip, stomion, lowerLip].filter((p): p is Vec3 => !!p);
  ...
  if (asymCount > 0 && bizygomatic > 0) metrics.fluctuating_asymmetry = { value: (asymSum / asymCount) / bizygomatic, raw: { pairs: asymCount, mirrorX } };
  ...
  if (!suppressExpressive && !missingLoops) {
    ...
    if (upperLidL && lowerLidL && innerCL && outerCL && upperLidR && lowerLidR && innerCR && outerCR) {
      ...
      if (wL > 0 && wR > 0) metrics.palpebral_fissure_ratio = { value: ((hL / wL) + (hR / wR)) / 2, raw: { hL, wL, hR, wR } };
    }
    ...
    if (irisL && irisR && innerCL && outerCL) {
      ...
      const eyeWidth = distance(innerCL, outerCL);
      if (eyeWidth > 0) metrics.ipd_ratio = { value: ipd / eyeWidth, raw: { ipd, eyeWidth, source: 'canthus_proxy' } };
    }
  }
  ...
  if (!suppressExpressive) {
    ...
    if (gonionL && gonionR && zygionL && zygionR) {
      ...
      if (bz > 0) metrics.bigonial_bizygomatic_ratio = { value: bigonial / bz, raw: { bigonial, bizygomatic: bz } };
    }
  ...
  if (tragionL && tragionR && zygionL && zygionR) {
    ...
    const bz = distance(zygionL, zygionR);
    if (bz > 0) metrics.bitemporal_bizygomatic_ratio = { value: bitemporal / bz, raw: { bitemporal, bizygomatic: bz } };
  }
  ...
  if (zygionL && zygionR && tragionL && tragionR) {
    ...
    const bz = distance(zygionL, zygionR);
    if (bz > 0) metrics.zygomatic_projection = { value: (zygZ - tragZ) / bz, raw: { zygZ, tragZ, bizygomatic: bz } };
  }

  if (glabella && menton && zygionL && zygionR) {
    const faceHeight = Math.abs(glabella.y - menton.y);
    const bz = distance(zygionL, zygionR);
    if (bz > 0) metrics.facial_index = { value: faceHeight / bz, raw: { faceHeight, bizygomatic: bz } };
  }
  ...
  if (!suppressExpressive && upperLip && stomion && lowerLip) {
    ...
    if (lower > 0) metrics.lip_ratio = { value: upper / lower, raw: { upper, lower } };
  }
  ...
  if (alarL && alarR && zygionL && zygionR) {
    ...
    const bz = distance(zygionL, zygionR);
    if (bz > 0) metrics.alar_bizygomatic_ratio = { value: alar / bz, raw: { alar, bizygomatic: bz } };
  }

  if (!suppressExpressive && cheilionL && cheilionR && alarL && alarR) {
    const mouth = distance(cheilionL, cheilionR);
    const alar = distance(alarL, alarR);
    if (alar > 0) metrics.mouth_nose_ratio = { value: mouth / alar, raw: { mouth, alar } };
  }
...
export function resolveIdeal(metricKey: string, sex: 'male' | 'female' | null): PointIdeal | RangeIdeal | null {
...
export function scoreFromIdeal(value: number | null | undefined, ideal: PointIdeal | RangeIdeal | null): number | null {
  ...
  if (isRangeIdeal(ideal)) {
    ...
    if (value < idealMin) return Math.round(85 * (value - hardMin) / (idealMin - hardMin));
  ...
  if (isPointIdeal(ideal)) {
    ...
    if (dev <= halfWidth) return Math.round(100 - 15 * (dev / halfWidth));
...
export function scoreMetrics(metrics: Record<string, MetricEntry>, sex: 'male' | 'female' | null = null): Record<string, number> {
...
export function composeHarmonyScore(
...
): { harmony: number | null; domainScores: Record<BoneDomain, number | null>; presentDomains: number } {
  let total = 0;
  let weightUsed = 0;
  ...
  for (const [domain, weight] of Object.entries(DOMAIN_WEIGHTS) as Array<[BoneDomain, number]>) {
    ...
    if (present.length === 0) { domainScores[domain] = null; continue; }
  ...
  if (confidence === 'low' || presentDomains < 4 || weightUsed === 0) return { harmony: null, domainScores, presentDomains };
...
interface FindingRule { metric: string; direction: FindingDirection; code: BoneFindingCode; }
...
export function findingsFromScores(metrics: Record<string, MetricEntry>, scored: Record<string, number>, sex: 'male' | 'female' | null): BoneFinding[] {
  ...
  for (const rule of FINDING_RULES) {
    ...
    if (isRangeIdeal(ideal)) side = metric.value < ideal.idealMin ? 'low' : metric.value > ideal.idealMax ? 'high' : 'either';
    else if (isPointIdeal(ideal)) side = metric.value < ideal.target ? 'low' : metric.value > ideal.target ? 'high' : 'either';
...
export interface AnalyzeBoneInput {
  ...
  blendShapes?: Record<string, number> | null;
  sex?: 'male' | 'female' | null;
  source?: BoneMeshSource | 'canonical';
}
...
export function analyzeBoneStructure({ vertices, indices = null, blendShapes = null, sex = null, source = 'arkit' }: AnalyzeBoneInput): BoneAnalysis {
  const normalized = normalizeSource(source);
  if (isDegenerateMesh(vertices)) {
    return { harmony: null, status: 'no_face', metrics: {}, scored: {}, findings: [], domainScores: {} as Record<BoneDomain, number | null>, dominantDriver: null, source: normalized, sex: sex || null, estimate: normalized !== 'arkit', confidence: 'low', landmark_source: normalized === 'arkit' ? 'derived' : 'template' };
  }
  const resolution = resolveLandmarks(vertices, normalized, indices);
  if (resolution.status !== 'ok') {
    return { harmony: null, status: 'no_face', metrics: {}, scored: {}, findings: [], domainScores: {} as Record<BoneDomain, number | null>, dominantDriver: null, source: normalized, sex: sex || null, estimate: normalized !== 'arkit', confidence: resolution.confidence, landmark_source: resolution.landmark_source };
  }
  ...
  if (!metrics) {
    return { harmony: null, status: 'no_face', metrics: {}, scored: {}, findings: [], domainScores: {} as Record<BoneDomain, number | null>, dominantDriver: null, source: normalized, sex: sex || null, estimate: normalized !== 'arkit', confidence, landmark_source: resolution.landmark_source };
  }
  ...
  const status: 'ok' | 'insufficient' = harmony == null || confidence === 'low' || presentDomains < 4 ? 'insufficient' : 'ok';
