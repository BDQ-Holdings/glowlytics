...
export default function LandingPage() {
  ...
  const formOkRef = useRef<HTMLDivElement>(null);
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    ...
    const ok = formOkRef.current;
    const errorEl = formErrorRef.current;
    if (!form || !input || !btn || !note || !ok) return;
    ...
    fetch("https://glowlytics-api-production.up.railway.app/api/waitlist", {
      method: "POST",
      ...
      .catch(() => {
        let saved = false;
        try {
          const list = JSON.parse(localStorage.getItem("gl_wl") || "[]");
          list.push({ email, t: new Date().toISOString() });
          localStorage.setItem("gl_wl", JSON.stringify(list));
          saved = true;
        } catch {
        ...
        const title = ok.querySelector(".form-ok-title");
        const sub = ok.querySelector(".form-ok-sub");
        if (title) title.textContent = "Almost there!";
        if (sub)
          sub.textContent =
            "We saved your email. We\u2019ll confirm your spot as soon as our servers sync.";
      })
      .finally(() => {
  ...
  return (
    <div className="landing-page">
      ...
      <section className="cta" id="access" ref={ctaRef}>
        <div className="cta-inner">
          ...
          <p className="cta-sub reveal rd2">
            ...
            We&apos;ll send you a free personalized skin report on launch day.
          </p>
          <p
          ...
          >
            Coming to iOS &middot; Spring 2026 &middot; Founding members get
            lifetime pricing
          </p>
          ...
          <div className="form-wrap reveal rd3">
            ...
            <p className="form-note" ref={formNoteRef}>
              No spam, ever. Unsubscribe in one click.
            </p>

            <div className="form-error" ref={formErrorRef} role="alert">
              Something went wrong. Please try again in a moment.
            </div>
            <div className="form-ok" ref={formOkRef}>
              ...
              <div className="form-ok-title">You&apos;re on the list.</div>
              <div className="form-ok-sub">
                We&apos;ll reach out when Glowlytics opens, plus a founding
                member welcome gift.
              </div>
            </div>
          </div>
      ...
      <div className="sticky-cta" ref={stickyRef}>
        <a href="#access">Start Free Trial, No Credit Card</a>
