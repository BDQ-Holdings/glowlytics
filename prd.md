...
- **PostgreSQL** database
- **MCP (Model Context Protocol) server** — Streamable HTTP transport mounted at `/mcp` with JWKS Bearer auth, beta allowlist gate, per-user rate limiter (60/min + 10/sec burst), structured tool-call audit logging. Exposes 11 user-scoped tools to authenticated MCP clients (Claude.ai etc.): `get_latest_scan`, `get_scan_history`, `get_signal_trend`, `compare_scans`, `get_scan_report`, `get_current_routine`, `lookup_ingredient`, `search_ingredients`, `summarize_month`, `get_bone_structure`, `get_harmony_trend`. Connected apps managed via `/api/mcp/clients` GET + DELETE; surfaced in the Profile → Connected Apps (Beta) section.
...
- **MCP server** — Streamable HTTP transport at `/mcp`, JWKS Bearer auth, beta allowlist, per-user rate limiter (60/min + 10/sec burst), 11 user-scoped tools (`get_latest_scan`, `get_scan_history`, `get_signal_trend`, `compare_scans`, `get_scan_report`, `get_current_routine`, `lookup_ingredient`, `search_ingredients`, `summarize_month`, `get_bone_structure`, `get_harmony_trend`), structured tool-call audit logging, cross-tool user-scoping test suite. Connected Apps (Beta) section in Profile lists + revokes clients via `/api/mcp/clients` GET+DELETE.
...
- **Bone structure / Harmony analysis** — full 3D facial-architecture feature. Backend math (`backend/bone-structure-3d.js`) computes 16 anatomical metrics across 6 weighted domains (Symmetry 25, Periorbital 20, Mandibular 20, Midface 15, Nose 10, Brow 10) producing a 0–100 Harmony composite, sex-aware ideals for `gonial_angle`/`nasolabial_angle`/`brow_position`. Findings map to three-tier interventions (Lifestyle / Pharmacological / Procedural) with a strong procedural disclaimer (`backend/interventions.js`). Capture: `expo-arkit-face` Swift native module for TrueDepth devices + canonical fallback (`src/services/canonicalFaceMesh.ts` — 32 Hyper3D Rodin-sampled landmarks via Blender MCP, ~65 outline edges, 42 triangle faces, axis-swapped + symmetry-enforced). Renderer: `src/components/Face3DViewer.tsx` (SVG + JS perspective projection with `w = distance - z2` + x-mirror, pan/pinch/orbit, idle RAF auto-rotate at ~17°/sec, clay-sculpt triangle fills via painter's algorithm + back-half culling, screen-fixed fake key-light from upper-left, depth-graded edges, revealProgress prop, four modes: anatomy / heatmap / measurements / skin). Capture screen `app/scan/bone-capture.tsx`: 5-stage progress ring + breathing chip dots + staged haptics (Light→Medium→Selection→Success) + copy crossfade + done badge + sex-baseline prompt. Results screen `app/scan/bone-results.tsx`: 5-page vertical story carousel (hero · measurements · by-area · findings · interventions). Today-screen surfaces: `HarmonyTrendCard` (sparkline + delta vs first scan), `HarmonyFocusCard` (top finding + one lifestyle action). Reports tab: Facial architecture section. `account.tsx`: ArchitectureLauncher with trailing Harmony badge. Persistence via `model_outputs.bone_structure JSONB` (migration v4). No-LLM mode (`backend/no-llm-fallback.js`) lets the analyze pipeline run on deterministic L1+L2 + template insights when `OPENAI_DISABLED` or the OpenAI key is absent.
- **Production builds submitted** — v1.1.6 build #106 (2026-05-09), #107 (2026-05-13, OAuth DCR proxy + monorepo rename + mcpClients httpJson migration), #117 (2026-05-15, sign-in crash hotfix from unstable Zustand selectors), #118 (2026-05-16, 3D mesh viewer fixes: projection from back-of-head to front, clay-sculpt triangle fills, key-light shading, RAF auto-rotate, heatmap halo+core), #119 (2026-05-16, Scan Flow + Settings Flow design hand-offs — face-map results screens, settings hub + 9 subscreens), #120 (2026-05-17, AuthRedirector route-allowlist fix so the new settings and architecture screens no longer bounce to Today). Next planned submission: **v1.2.0 (build 1)** after the local-appVersionSource switch (`ba09974`) and the ritual rewire (`ef67e14`) — ritual route moved /today/ritual → /ritual to dodge a resolver collision, derives steps from the user's shelf, persists daily check-offs.
- **Scan Flow design implementation** (commit `a6401d5`) — new SVG primitive `src/components/scan/FaceMapZones.tsx` (face silhouette + 7 toggleable zones, optional sweep line, annotated landmarks, warm halo) and atoms `ScanAtoms.tsx` (Chip dark-glass + cream variants, Dot, StagePills). Three new results screens: `app/scan/face-map.tsx` (face-map hero + by-area cards backed by signal_scores), `app/scan/zone-detail.tsx` (ghosted face, "what we noticed", 2-week sparkline), `app/scan/method.tsx` (three-source transparency with weighted bars + confidence). Camera framing overlay gained the "5 of 5 landmarks" lock chip + italic *"We've found you"* copy; analyzing screen gained the Captured → Mapped → Compared → Composed StagePills strip and four orbiting findings chips that fade in by stage. Existing 3-layer ML pipeline unchanged.
- **Settings Flow design implementation** (commit `d0e15eb`) — dedicated `/settings` stack with shared primitives in `src/components/settings/SettingsPrimitives.tsx` (SettingsPage, SettingsHeader, ListGroup, Row, Toggle, Chip, Pill, PrimaryButton, GhostButton, SectionLabel). Ten screens: hub (`index.tsx`) with profile hero + four sections, skin-profile (type chips + sensitivity multi-chips + ranked concerns), notifications ("two nudges a day, never more" with routine + nudges + quiet hours + email), privacy ("Your face never leaves your phone" pledge hero on dark plum), camera (storage card with 3-segment bar + capture toggles + muted-danger wipe), appearance (Dusk/Meadow/Rose/Auto palette grid + Light/Dark/Auto + text-size slider + app-icon picker), export (checkbox items with sizes + PDF/Raw/Markdown format pills), help (search + top questions + freeform feedback with type chips + reach-a-human + status row), about (logo + why card + legal + credits with version from `expo-constants`), delete-account (warning hero + what-goes-away + two off-ramps + reason chips + danger CTA with Alert). Me-tab `SETTINGS_ROWS` rewired to these new screens.
- **AuthRedirector route allowlist fix** (commit `546efda`) — `app/_layout.tsx`'s `inLoggedInRoute` whitelist gates which top-level segments a signed-in user may stay on. Six routes were missing — `settings`, `architecture`, `account`, `routine`, `story`, `today` — so tapping rows that pushed onto those stacks mounted briefly then bounced back to `/(tabs)/today`. Whitelist now covers every top-level app route.
- **Ritual rewire to shelf** (commit `ef67e14`, v1.2.0 bump) — moved `/today/ritual` → `/ritual` to dodge a resolver collision with `(tabs)/today.tsx` that was bouncing pushes back to home. New `src/services/ritual.ts` (95 lines) derives ritual steps from `useStore.products` (Morning/Evening/Both), plus water + SPF habits when no sunscreen is on shelf. Persistent daily check-offs live in `ritualCompletions` keyed by local date; tap toggles, long-press product row removes from shelf. Inline `AddProductSheet` on ritual screen for write-from-ritual.
- **EAS local appVersionSource** (commit `ba09974`) — eas.json switched from `appVersionSource: remote` + `autoIncrement: true` to `local` + `autoIncrement: false`. The remote registry kept incrementing from 120; lets `app.json` / `Info.plist` / `project.pbxproj` drive the shipped version+build so v1.2.0 (build 1) lands cleanly on TestFlight.
- **Pattern surfaces — Glow repalette + duplication fix** — `PatternCard`, `PatternCarousel`, and `PatternProgressBar` migrated from the legacy teal `Colors.primary` stack onto the Glow palette so the bottom of the Today screen matches the rest of the app. Carousel header now uses `<SectionHead>` for rhythm consistency. `home.tsx` restructured so the featured pattern only renders once (Glow-styled "Your story" card) instead of twice (story card + first carousel slot). `PatternProgressBar` folded into the story card so the "Day X of 14" maturity primer is anchored to the pattern it describes. Carousel only renders when there's more than one pattern, filtered to exclude the featured one.
- **HealthKit driver expansion** — `patternEngine.detectHealthSignalLag` driver list extended from 3 → 7. `sleep_deep_minutes`, `sleep_rem_minutes`, `steps`, and `mindful_minutes` were already being pulled by `healthSync.pullLastNDays` and stored in `healthDailyRecords` but never read by the engine. Correlation search space grew 12 → 28 candidates at zero additional data-collection cost.
- **HealthKit rollup to L3 insights** — new `healthSync.buildHealthkitRollup(records, windowDays)` pure helper returns 7-day averages of sleep total/deep/REM, HRV, RHR, steps, mindful, plus most-severe menstrual flow in window and today's cycle day. `visionAPI.streamInsights` gained a typed `healthkit_context` payload; `scan/analyzing.tsx` builds the rollup and passes it; `backend/app.js buildInsightPrompt` renders a conditional **HealthKit signals** block in the L3 GPT-4o system prompt with formatted values (e.g. "Sleep (avg over last 7d): 6h 12m, 1h 4m deep, 1h 22m REM") and an instruction to ground pattern-level copy in those values rather than fabricate trends. Block is omitted entirely when every signal is null so prompts stay clean for users without HealthKit.
- **HealthKit post-onboarding backfill** — `app/account.tsx handleHealthConnect` now branches on whether `healthDailyRecords` is empty: first-time grant outside onboarding → `syncHealthDataInitial` (14-day backfill, primes pattern detection immediately); subsequent re-grants → `syncHealthData` (2-day delta). Closes the gap where users who skipped HealthKit during onboarding and connected later sat with only 2 days of paired data and couldn't surface confident patterns.
- **747 tests across 56 suites** (combined mobile + backend when run from `apps/glowlytics`), 0 TypeScript errors. Two pre-existing infra-only suite failures (an empty fixtures file is matched as a test, and `backend/__tests__/mcp/protocol-e2e.test.js` requires the now-missing `pkce-challenge` dep from `@modelcontextprotocol/sdk`).
- 74 screen files (63 + 11 _layouts) across `app/`, 40 top-level components + `glow/` + `navigation/` + `scan/` (FaceMapZones, ScanAtoms) + `settings/` (SettingsPrimitives), 35 services, 10 top-level backend modules + 12 `mcp/` + 5 `queries/` helpers, Zustand store with persisted ritualCompletions
